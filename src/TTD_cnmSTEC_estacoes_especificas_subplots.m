...
%     t_fim1    = datenum(2025,8,22,3,06,0)- 678942;   % 22 Aug 03:06 (50 minutos)
    ...
    % condicional caso alguma estação não tenha o prn escolhido
    if isempty(time_f)
        disp(['Estação ', file, ' não possui PRN ', num2str(prn_escolhido), ' -> pulando'])
        i = i + 1;
        continue
    end
...
%    s.SizeData = 75;
 ...
 h = gobjects(length(x)-1,1); % pré-alocar array para handles
    ...
    resp_str = input(['Manter estação ', file, '? (1 = sim, 0 = nao, -1 = para retornar para o anterior, 9 = sair): '], 's');
...
figure(2)
clf
set(gcf, 'Position', [100 100 600 850]) % altera o tamanho do gráfico [left  bottom  width  height]

n = length(selected_data);

if n == 0
    disp('Nenhuma estação foi selecionada.')
else
    for i_plot = 1:n
...
%       title(['Estação: ', selected_files{i_plot}])
%         xlabel('Time (UT)')

        % plota só os primeiros 4 caracteres no nome das estações em caps lock
        if i_plot == n
            xlabel('Time (UT)')
        end
        nome = selected_files{i_plot};
        ylabel(upper(nome(1:4)))
    end
end
